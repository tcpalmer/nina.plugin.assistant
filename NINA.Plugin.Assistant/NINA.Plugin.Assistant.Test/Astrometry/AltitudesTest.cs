...
namespace NINA.Plugin.Assistant.Test.Astrometry {
    ...
    public class AltitudesTest {
        ...
        public void TestClipAscendingStart() {
            ...
            Assert.That(ex.Message, Is.EqualTo("altitude list is unexpectedly always ascending"));
        }
        ...
        public void TestBad() {
            ...
            Assert.That(ex.Message, Is.EqualTo("altitudes cannot be null"));

            List<AltitudeAtTime> altitudes = new List<AltitudeAtTime>();
            ex = Assert.Throws<ArgumentException>(() => new Altitudes(altitudes));
            Assert.That(ex.Message, Is.EqualTo("altitudes must have at least two values"));
            ...
            ex = Assert.Throws<ArgumentException>(() => new Altitudes(altitudes));
            Assert.That(ex.Message, Is.EqualTo("time is not always increasing"));
        }
    }
    ...
    public class AltitudeAtTimeTest {
        ...
        public void TestBad() {
            ...
            Assert.That(ex.Message, Is.EqualTo("altitude must be <= 90 and >= -90"));

            ex = Assert.Throws<ArgumentException>(() => new AltitudeAtTime(45, 361, DateTime.Now));
            Assert.That(ex.Message, Is.EqualTo("azimuth must be >= 0 and <= 360"));
        }
    }
}